  ...
  const handleSearchNewMember = (e) => {
    ...
    if (str.trim().length > 0) {
      const list = friendList.filter(
        (friend) =>
          friend.fullName.toLowerCase().includes(str) ||
          friend.username.toLowerCase().includes(str)
      );
                  ...
                  <li
                    ...
                    onClick={() => {
                      const isPresent = memberList.some((mem) => {
                        return (
                          mem.fullName.toLowerCase() ===
                            member.fullName.toLowerCase() ||
                          mem.username.toLowerCase() ===
                            member.username.toLowerCase()
                        );
                      });
                      if (!isPresent) {
                        setMemberList((prevState) => [...prevState, member]);
                      }
                      suggestInput.current.value = "";
                      suggestList.current.classList.remove("show");
                      setSuggestMember([]);
                  ...
                  <tr
                    ...
                    onClick={() => {
                      setMemberList((prevState) =>
                        prevState.filter((mem) => mem !== member)
                      );
                    }}
                  >
